...
interface PromotionalWhatsAppButtonProps {
    products: Product[];
}

const PromotionalWhatsAppButton: React.FC<PromotionalWhatsAppButtonProps> = ({ products }) => {
    
    const handlePromoClick = () => {
        // TODO: Substitua pelo seu número de WhatsApp com código do país (ex: 5511912345678)
        const phoneNumber = "5511912345678"; 
        const courseList = products.map(p => `- ${p.name}`).join('\n');
        const message = encodeURIComponent(`Olá! Gostaria de apresentar nossos cursos de transformação de carreira. Temos as seguintes opções disponíveis:\n\n${courseList}\n\nQual deles mais te interessa?`);
    ...
    return (
        ...
        >
            <WhatsAppIcon />
            <span className="ml-2">Oferecer Cursos via WhatsApp</span>
        </button>
    );
};

export default PromotionalWhatsAppButton;
